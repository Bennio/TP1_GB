			...
			case 'A':
				...
				try {
					...
					if (trouver == false) {
						...
						Transactions trans = new Transactions(numTrans, cpte.getIdClient(), cpte.getIdCompte(),
								dateTrans, typeTrans, montantTrans, solde_avant_t, solde_apres_t);
						listeTransaction.add(trans);

						System.out.println("\nLes caracteristiques du compte sont:");
						for (Comptes comptes : listeCpte) {
							System.out.println(comptes);
						}

						System.out.println("\nLes informations de la transaction sont: ");
						for (Transactions transactions : listeTransaction) {
					...
					if (listeCpte.isEmpty()) {
						...
						for (Comptes cpte_tenu : listeCpte) {
							...
							if (tempCompte == numCompte) {
								...
								numTrans = listeTransaction.size() + 1;
								Transactions trans = new Transactions(numTrans, cpte_tenu.getIdCompte(),
										cpte_tenu.getIdClient(), dateTrans, typeTrans, montantTrans, solde_avant_t,
										solde_apres_t);
								listeTransaction.add(trans);
								System.out.println("Vous aviez " + solde_avant_t + " sur votre compte, \n"
										+ "vous venez de faire un depot de: " + montant + ", presentement vous avez: "
										+ solde_apres_t + " sur votre compte");
								System.out.println("\n");
				...
				try {
					...
					if (listeCpte.isEmpty()) {
						...
						for (Comptes cpte_tenu : listeCpte) {
							...
							if (tempCompte == numCompte) {
								System.out.println(
										"Entrez le montant a enlever - NB: un montant positif inferieur a votre solde");
								montant = lecture.nextDouble();
								...
								listeTransaction.add(trans);
								System.out.println("Vous aviez " + solde_avant_t + " sur votre compte, \n"
										+ "vous venez de faire un retrait de: " + montant + ", presentement vous avez: "
										+ solde_apres_t + " sur votre compte");
								System.out.println("\n");
			...
			case 'G':
				try {
					for (int x = 0; x < listeCpte.size(); x++) {
						listeCpte.get(x)
								.setSoldeCompte(listeCpte.get(x).getSoldeCompte() * (1 + listeCpte.get(x).getTaux()));
						System.out.println("");
